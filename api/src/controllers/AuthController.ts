...
async function sendEmail(
  ...
  await sgMail.send({
    to,
    from: process.env.SENDGRID_FROM_EMAIL || 'noreply@vistaq.co',
    subject,
    html,
  });
}
...
async function forgotPassword(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const frontendUrl = process.env.FRONTEND_URL || 'https://vistaq.co';
    const resetLink = `${frontendUrl}/reset-password?token=${token}`;

    const html = `
<p>Hi ${userData.name},</p>
<p>You requested to reset your VistaQ password.</p>
<p>Click the link below to reset your password (valid for 1 hour):</p>
<p><a href="${resetLink}">Reset Password</a></p>
<p>If you didn't request this, please ignore this email.</p>
<p>Best regards,<br>VistaQ Team</p>
    `.trim();

    await sendEmail(userData.email as string, 'Reset Your VistaQ Password', html);

    console.log(`[ForgotPassword] Reset email sent to ${userData.email}`);
...
async function resetPassword(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Check expiry
    const expires = userData.passwordResetExpires as admin.firestore.Timestamp;
    ...
    console.log(`[ResetPassword] Password reset for user ${userDoc.id}`);
